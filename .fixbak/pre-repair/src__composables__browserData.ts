 ...
 * Electron 妯″ʹ�� SQLite；当应用通过 Vite 直接打开或运行在不带 preload 鐨? * 浏����腑鏃讹紝浠嶇劧闇一要一套行为一致的 CRUD / 查������������������������ҳ��һ����″��页会�一化成һ�这里故意只实现受限的查询跨������璇㈣��法，避免�? 任意绘剰 SQL��¶������������ʱһ一? */
...
const TABLE_DEFINITIONS: Record<string, Omit<BrowserTableDefinition, 'rows'>> = {
  ...
  customers: {
    ...
    fields: [
      ...
      field('id', 'INTEGER', '��������?', { isPrimaryKey: true, isNotNull: true }),


      field('name', 'TEXT', '客�����', { isNotNull: true }),


      field('contact', 'TEXT', '��ϵ?'),


      field('phone', 'TEXT', '联系电话'),


      field('status', 'TEXT', '客�״һ?'),
  ...
  orders: {
    ...
    fields: [
      ...
      field('order_no', 'TEXT', '订�����', { isNotNull: true }),


      field('customer_id', 'INTEGER', '�����户'),


      field('amount', 'REAL', '订�����'),


      field('status', 'TEXT', '订�״һ?'),
  ...
  tickets: {
    ...
    fields: [
      ...
      field('title', 'TEXT', '工�����', { isNotNull: true }),
...
const SEED_ROWS: Record<string, Record<string, unknown>[]> = {
  ...
  customers: [
    ...
    { id: 1, name: '星河科技', contact: '闄堟陈晨 phone: '138****1201', status: '已成��ѳ� owner: '鏋楁檽林晓created_at: '2026-08-18 09:20:00' },


    { id: 2, name: '云帆网络', contact: '鏉庢李想 phone: '139****2816', status: '璺熻繘涓?', owner: '鍛ㄨ埅', created_at: '2026-08-19 14:05:00' },


    { id: 3, name: '北辰贸易', contact: '鐜嬫王楠 phone: '136****8302', status: '已成��ѳ� owner: '鏋楁檽林晓created_at: '2026-08-20 10:32:00' },


    { id: 4, name: '原野设计', contact: '赵晴', phone: '135****6118', status: '寰呰仈绯?', owner: '许��', created_at: '2026-08-21 08:48:00' },
  ...
  orders: [
    ...
    { id: 1, order_no: 'SO-20260818-001', customer_id: 1, amount: 12800, status: '����?', created_at: '2026-08-18 10:10:00' },
  ...
  tickets: [
    ...
    { id: 1, title: '���������һҪ��修�?', priorit普�һ'鏅一?', assigne�� '许�█', status: '澶勭悊涓?', updated_at: '2026-08-20 16:20:00' },


    { id: 2, title: '订嵽�������쳣', priority: '绱ф一?', assignee: '鍛ㄨ埅', status: '待处��?', updated_at: '2026-08-21 09:05:00' },


    { id: 3, title: '�����充产�˵��', priority: '浣?', assignee: '林晓', status: '已�ѧ���', updated_at: '2026-08-21 10:40:00' },
...
function readStore(): StoredData {
  ...
  try {
    ...
    // localStorage ���ܱ���˽��ʽ�������略��ã�һ���ڴ�����һㄥ��可�一?
...
function assertTable(tableName: string) {
  ...
  if (!TABLE_DEFINITIONS[tableName]) throw new Error(`���������?{tableName}`)
...
function applyWhere(source: Record<string, unknown>[], tableName: string, where?: string) {
  ...
  const predicates = expressions.map(expression => {
    ...
    if (!match || !names.has(match[1])) throw new Error(`������֧�֣�：${expression}`)
...
function writableData(tableName: string, data: Record<string, unknown>) {
  ...
  if (!entries.length) throw new Error('娌℃��д������')
...
function browserBootstrap(): BootstrapData {
  ...
  if (projects.length) return { projects, activities: [], databasePath: '����ʾ����寮?· localStorage' }
  ...
  return { ...fallback, projects: normalized, databasePath: '����ʾ����寮?· localStorage' }
...
export function browserSaveProject(project: LowCodeProject) {
  ...
  if (!project?.id || !project?.name || !project?.layout) throw new Error('Ӧ�����ݲ���?')
...
function browserDeleteProject(projectId: string) {
  ...
  if (!projects.some(project => project.id === projectId)) throw new Error('Ӧ�ò���?')
