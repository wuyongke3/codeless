...
async function clickWidget(id, shiftKey = false) {
  ...
  return inRenderer((widgetId, additive) => {
    ...
    if (!element) throw new Error(`�Ҳ�����?${widgetId}`)
...
async function clickMenu(command) {
  ...
  return inRenderer(commandValue => {
    ...
    if (button.disabled) throw new Error(`�Ҳ����˵���?${commandValue}`)
...
async function main() {
  ...
  await runCase('canvas loading and baseline widgets', async () => {
    ...
    assert.equal(new Set(snapshot.map(item => item.id)).size, snapshot.length, '组件 ID ��Ψһ')
  ...
  await runCase('design exchange import and export entry points', async () => {
    ...
    assert.equal(controls.importExists, true, '缺少设计 JSON �����口')


    assert.equal(controls.exportExists, true, '缺少设计 JSON �����口')
    ...
    assert.deepEqual(errors, [], `���Ƥ换入口产生 renderer error锛?{errors.join('; ')}`)
  ...
  await runCase('canvas grid and smart snap', async () => {
    ...
    assert.equal(initial.gridExists, true, '����һ�ز�����')
    ...
    assert.equal(hidden.active, false, '�����رպ���ť��?active')
    ...
    assert.equal(await inRenderer(() => document.querySelector('[data-testid="canvas-snap-toggle"]')?.classList.contains('active') || false), false, '关闭吸附后按钮仍��?active')
  ...
  await runCase('multi-select alignment and undo', async () => {
    ...
    assert.equal(selected, 3, `��һ���������쳣��${selected}`)
    ...
    assert.equal(new Set(after).size, 1, `左���������һ致：${after.join(', ')}`)


    assert.ok(new Set(before.map(item => item.left)).size > 1, `���Գ�ʼλ��δ�γɿ���֤����?{JSON.stringify(before)}`)
    ...
    assert.deepEqual(undone, before.map(item => item.left), `左�������δ����һ��?{JSON.stringify({ before, undone })}`)
  ...
  await runCase('canvas marquee selection', async () => {
    ...
    assert.ok(selected >= 1, `��һδ����������selected=${selected}`)
  ...
  await runCase('component panel coverage', async () => {
    ...
    assert.ok(allTypes.length >= 20, `���������������쳣?{allTypes.length}`)
    ...
    for (const type of allTypes) {
      ...
      await inRenderer(typeValue => {
        ...
        if (!button) throw new Error(`�����板缺少 ${typeValue}`)
    ...
    assert.equal(new Set(ids).size, ids.length, '�������Ӻ���?ID 重复')
  ...
  await runCase('Element Plus component rendering', async () => {
    ...
    for (const item of checks) {
      ...
      assert.equal(item.rendered, true, `${item.type} �ڲ���Ⱦ�ڵ�ȱ��`)
    ...
    assert.equal(progress?.progressWidth, '68%', '���ȡ默认��ֱ�δӳ�䵽ㄩ������


    assert.equal(checks.find(item => item.type === 'switch')?.switchPressed, 'true', '��һ��Ĭ��̬δ��Ⱦ')


    assert.ok((checks.find(item => item.type === 'checkbox')?.checkboxCount || 0) >= 1, 'checkbox input is missing')


    assert.ok((checks.find(item => item.type === 'radio')?.radioCount || 0) >= 2, '��һ��ѡ��缺失')


    assert.ok((checks.find(item => item.type === 'pagination')?.paginationButtons || 0) >= 3, '�֡���ťȱ��')


    assert.equal(checks.find(item => item.type === 'tabs')?.activeTabs, 1, '��ǩҳû��Ψһ��һ活项')
  ...
  await runCase('runtime preview interactions', async () => {
    ...
    assert.equal(runtime.dateType, 'date', '日�ѡ��ㄦ����ʹate input')
    ...
    assert.equal(await inRenderer(() => document.querySelector('.runtime-widget-node.widget-checkbox input[type="checkbox"]')?.checked), true, '����̬��ѡ��ѡ��℃��选中')
    ...
    assert.ok(radioInputs.length >= 2, '����̬��ѡ��ѡ������')
    ...
    assert.equal(await inRenderer(() => document.querySelectorAll('.runtime-widget-node.widget-tabs .render-tabs-head button.active')[0]?.textContent?.trim()), '\u8be6\u60c5', '����̬��ǩ���л�ʧ��')
    ...
    assert.equal(await inRenderer(() => getComputedStyle(document.querySelector('.runtime-widget-node.widget-modal .render-modal')).display), 'flex', 'Modal Ĭ���Ԥ�ʧ��')
    ...
    assert.equal(await inRenderer(() => getComputedStyle(document.querySelector('.runtime-widget-node.widget-modal .render-modal')).display), 'none', 'Modal �رհ�ť℃���ص���')


    assert.equal(await inRenderer(() => getComputedStyle(document.querySelector('.runtime-widget-node.widget-drawer .render-drawer')).display), 'block', 'Drawer Ĭ���Ԥ�ʧ��')
    ...
    assert.equal(await inRenderer(() => getComputedStyle(document.querySelector('.runtime-widget-node.widget-drawer .render-drawer')).display), 'none', 'Drawer �رհ�ť℃���س���')


    assert.equal(await inRenderer(() => getComputedStyle(document.querySelector('.runtime-widget-node.widget-loading .render-loading')).display), 'flex', 'Loading ״һδ�Ԥ�')
  ...
  await runCase('context menu boundary scrolling and animation rules', async () => {
    ...
    assert.ok(menu.left >= 0 && menu.top >= 0 && menu.right <= 1440 && menu.bottom <= 1000, `�˵�Խ��?{JSON.stringify(menu)}`)


    assert.equal(menu.animated, true, 'δ���ֲ˵���뿪开�����则')
