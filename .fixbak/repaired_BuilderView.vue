  ...
  <section class="builder-layout" :style="{ '--component-panel-width': state.componentPanelWidth + 'px', '--inspector-panel-width': state.inspectorPanelWidth + 'px' }">
    ...
    <aside class="component-panel" :style="{ '--panel-width': `${state.componentPanelWidth}px` }">
      ...
      <template v-if="state.paletteTab === 'components'">
        ...
        <div class="panel-tip"><AppIcon name="cursor" :size="15" /><span>拖拽组件到画布；双击文字直接编辑；Shift ��һ��������������Ctrl/Cmd+D 复制</span></div>
      ...
      <div class="panel-resize-handle panel-resize-handle-right" :class="{ active: state.panelResizeSide === 'component' }" title="��ק��������������" @pointerdown.stop="state.startPanelResize($event, 'component')"></div>
    ...
    <div class="canvas-workspace" @wheel="state.handleCanvasWheel" @click="state.clearSelection()" @contextmenu.stop.prevent="state.handleCanvasContextMenu">
      ...
      <div ref="canvasViewportRef" class="canvas-stage" :class="{ 'is-panning': state.canvasViewport.isPanning }" @scroll="state.updateCanvasViewport"><div class="canvas-frame" :style="state.canvasViewport.contentStyle">
        ...
        <div ref="canvasRef" :class="['design-canvas', { 'large-project-canvas': state.largeProjectMode }]" :style="{ width: `${state.currentProject.layout.canvas.width}px`, height: `${state.currentProject.layout.canvas.height}px`, background: state.currentProject.layout.canvas.background, transform: `translate(${state.currentProject.layout.canvas.x || 0}px, ${state.currentProject.layout.canvas.y || 0}px)` }" @dragover.prevent="state.setDropTargetContainer('')" @drop.stop.prevent="state.onCanvasDrop" @pointerdown.capture="startCanvasPointer" @pointermove="state.handleCanvasPanPointerMove" @pointerup="state.handleCanvasPanPointerUp" @pointercancel="state.handleCanvasPanPointerCancel" @click.stop="state.handleCanvasClick" @contextmenu.stop.prevent="state.handleCanvasContextMenu">
          ...
          <div v-if="!state.currentProject.layout.widgets.length" class="empty-canvas"><span><AppIcon name="layers" :size="26" /></span><strong>�����拖入第��һ����</strong>Ҳ�ɥ���������������һ���ӵ�����到画布</p></div>
    ...
    <aside class="inspector-panel" :style="{ '--panel-width': `${state.inspectorPanelWidth}px` }">
      ...
      <template v-if="state.selectedWidget">
        ...
        <div class="panel-tabs inspector-tabs"><button :class="{ active: state.inspectorTab === 'properties' }" @click="state.inspectorTab = 'properties'">灞炴一</button><button :class="{ active: state.inspectorTab === 'events' }" @click="state.inspectorTab = 'events'">交互</button></div>


        <div v-if="state.inspectorTab === 'properties'" class="inspector-scroll">


          <section class="component-usage"><div><span><AppIcon name="info" :size="14" /></span><div><strong>鎬庝箞鐢</strong><p>{{ componentDefinition(state.selectedWidget.type).usage }}</p></div></div><small>统一协议 v1 · content / style / data / interaction</small></section>
          ...
          <section class="property-section"><div class="property-title"><span>����℃伅</span><AppIcon name="chevron-down" :size="14" /></div>


            <label class="property-field"><span>��������</span><input v-model="state.selectedWidget.name" @input="state.markDirty()" /></label>


            <label v-if="state.selectedWidget.type !== 'divider' && !['input', 'select'].includes(state.selectedWidget.type)" class="property-field"><span>{{ state.selectedWidget.type === 'stat' ? 'ָ������' : state.selectedWidget.type === 'image' ? '占位标题' :�Ԥ�����文字' }}</span><input v-model="state.selectedWidget.config.content.text" @input="state.syncWidget(state.selectedWidget)" /></label>
            ...
            <label v-if="state.selectedWidget.type === 'input'" class="property-field"><span>ռλ�᤺</span><input v-model="state.selectedWidget.config.content.placeholder" @input="state.syncWidget(state.selectedWidget)" /></label>
          ...
          <section class="property-section">
            ...
            <label class="property-field"><span>ǿ��</span><select :value="state.selectedWidget.config.style.tokenRefs?.accent || ''" @change="state.updateWidgetTokenRef('accent', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('color')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>


            <label class="property-field"><span>鑳屾櫙鑹</span><select :value="state.selectedWidget.config.style.tokenRefs?.background || ''" @change="state.updateWidgetTokenRef('background', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('color')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>


            <label class="property-field"><span>�ı�</span><select :value="state.selectedWidget.config.style.tokenRefs?.color || ''" @change="state.updateWidgetTokenRef('color', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('color')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>


            <label class="property-field"><span>圆角 Token</span><select :value="state.selectedWidget.config.style.tokenRefs?.borderRadius || ''" @change="state.updateWidgetTokenRef('borderRadius', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('radius')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>


            <label v-if="['heading', 'text', 'button', 'link', 'icon'].includes(state.selectedWidget.type)" class="property-field"><span>字号 Token</span><select :value="state.selectedWidget.config.style.tokenRefs?.fontSize || ''" @change="state.updateWidgetTokenRef('fontSize', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('typography')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>


            <label v-if="['card', 'frame', 'stack', 'grid', 'drawer'].includes(state.selectedWidget.type)" class="property-field"><span>�ڱ�?Token</span><select :value="state.selectedWidget.config.style.tokenRefs?.padding || ''" @change="state.updateWidgetTokenRef('padding', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('spacing')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>


            <label v-if="['stack', 'grid'].includes(state.selectedWidget.type)" class="property-field"><span>间隙 Token</span><select :value="state.selectedWidget.config.style.tokenRefs?.gap || ''" @change="state.updateWidgetTokenRef('gap', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('spacing')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>


            <label v-if="['card', 'frame', 'stack', 'grid', 'drawer'].includes(state.selectedWidget.type)" class="property-field"><span>阴影 Token</span><select :value="state.selectedWidget.config.style.tokenRefs?.shadow || ''" @change="state.updateWidgetTokenRef('shadow', $event)"><option value="">δ��</option><option v-for="option in tokenOptionsFor('shadow')" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>
          ...
          <section v-if="componentDefinition(state.selectedWidget.type).capabilities.dataBinding" class="property-section"><div class="property-title"><span>数据绑定</span><AppIcon name="chevron-down" :size="14" /></div>
            ...
            <label class="property-field"><span>绑定数据</span><select :value="state.selectedWidget.config.data.table || ''" @change="state.updateDataSource(state.selectedWidget, $event)"><option value="">��������ʹ�þ�һ��</option><option v-for="table in state.tables" :key="table.name" :value="table.name">{{ table.titl（{}}锛坽{ table.name }}锛</option></select></label>


            <template v-if="state.selectedWidget.config.data.table">


              <label class="property-field"><span>查询��″紡</span><select v-model="state.selectedWidget.config.data.mode" @change="state.syncWidget(state.selectedWidget)"><option value="list">�����С�</option><option value="single">单行数据</option><option v-if="state.selectedWidget.type === 'stat'" value="count��¼录�℃暟</option><option v-if="state.selectedWidget.type === 'stat'" value="aggregate�ۺϡ算＄畻</option></select></label>


              <div v-if="state.selectedWidget.type === 'select'" class="property-row"><label class="property-field"><span>�Ԥ��ֶ�</span><input v-model="state.selectedWidget.config.data.labelField" placeholder="name" @input="state.syncWidget(state.selectedWidget)" /></label><label class="property-field"><span>值字</span><input v-model="state.selectedWidget.config.data.valueField" placeholder="id" @input="state.syncWidget(state.selectedWidget)" /></label></div>
          ...
          <section v-if="state.selectedWidget.type === 'button'" class="property-section"><div class="property-title"><span>�����ύ交</span><AppIcon name="chevron-down" :size="14" /></div><label class="property-field"><span>提�ύ�����ݡ�/span><select :value="state.selectedWidget.config.submitTo?.table || ''" @change="state.updateSubmitTarget(state.selectedWidget, $event)"><option value="">不提</option><option v-for="table in state.tables" :key="table.name" :value="table.name">{{ table.title }（{坽{ table.name }}锛</option></select></label><small class="field-help"�����ӵ��ύ����ʾ�͵һ����һ������交互�一�中配置�ㄤ綔閾</small></section>
        ...
        <div v-else class="events-panel">
          ...
          <div class="events-panel-head"><div><span><AppIcon name="flow" :size="18" /></span><div><strong>�����互</strong><small�����¼��������ㄤ綔</small></div></div><button class="primary-button compact" @click="state.addWidgetEvent()"><AppIcon name="plus" :size="14" />添加事件</button></div>


          <div v-if="!state.selectedWidget.config.interaction.events?.length" class="events-empty"><AppIcon name="cursor" :size="22" /><p>Ϊ�������������¼�һ�表��֧��һ�е���ʱһ����������������֧��һ��һ�仯ʱ</p></div>
