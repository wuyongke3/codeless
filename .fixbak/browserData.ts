...
/**
 * 浏览�ㄩ檷绾фā式的数据适配�ㄣ一? *
 * Electron 妯″��使用 SQLite；当应用通过 Vite 直接打开或运行在不带 preload 鐨? * 浏览�ㄤ腑鏃讹紝浠嶇劧闇一要一套行为一致的 CRUD / 鏌ヨ��能力，否则组件的数据绑定�? * 数据�″��页会�一化成“只能看、不能用”�一傝繖閲屾晠鎰忓彧瀹炵幇鍙楅檺鐨勬煡璇㈣��法，避免�? * 浠绘剰 SQL 暴露给浏览器运行时�一? */
...
const TABLE_DEFINITIONS: Record<string, Omit<BrowserTableDefinition, 'rows'>> = {
  customers: {
    ...
    color: '#665cf6',
    fields: [
      field('id', 'INTEGER', '主键，自�?', { isPrimaryKey: true, isNotNull: true }),
      field('name', 'TEXT', '瀹㈡��名�О', { isNotNull: true }),
      field('contact', 'TEXT', '联系�?'),
      field('phone', 'TEXT', '联系电话'),
      field('status', 'TEXT', '瀹㈡��状�一?'),
  ...
  orders: {
    ...
    fields: [
      field('id', 'INTEGER', '主键，自�?', { isPrimaryKey: true, isNotNull: true }),
      field('order_no', 'TEXT', '璁㈠��编号', { isNotNull: true }),
      field('customer_id', 'INTEGER', '关联�㈡埛'),
      field('amount', 'REAL', '璁㈠��金额'),
      field('status', 'TEXT', '璁㈠��状�一?'),
      field('created_at', 'DATETIME', '创建时间'),
    ],
  },
  tickets: {
    ...
    fields: [
      field('id', 'INTEGER', '主键，自�?', { isPrimaryKey: true, isNotNull: true }),
      field('title', 'TEXT', '宸ュ��标题', { isNotNull: true }),
      field('priority', 'TEXT', '优先�?'),
      field('assignee', 'TEXT', '澶勭悊浜?'),
      field('status', 'TEXT', '澶勭悊鐘舵一?'),
...
const SEED_ROWS: Record<string, Record<string, unknown>[]> = {
  customers: [
    { id: 1, name: '鏄熸渤绉戞妧', contact: '闄堟櫒', phone: '138****1201', status: '已成�?', owner: '鏋楁檽', created_at: '2026-08-18 09:20:00' },
    { id: 2, name: '浜戝竼缃戠粶', contact: '鏉庢兂', phone: '139****2816', status: '璺熻繘涓?', owner: '鍛ㄨ埅', created_at: '2026-08-19 14:05:00' },
    { id: 3, name: '鍖楄景璐告槗', contact: '鐜嬫', phone: '136****8302', status: '已成�?', owner: '鏋楁檽', created_at: '2026-08-20 10:32:00' },
    { id: 4, name: '原野设计', contact: '璧垫櫞', phone: '135****6118', status: '寰呰仈绯?', owner: '许�█', created_at: '2026-08-21 08:48:00' },
  ],
  orders: [
    { id: 1, order_no: 'SO-20260818-001', customer_id: 1, amount: 12800, status: '已完�?', created_at: '2026-08-18 10:10:00' },
  ...
  tickets: [
    { id: 1, title: '发�エ淇℃伅闇一要修�?', priority: '鏅一?', assignee: '许�█', status: '澶勭悊涓?', updated_at: '2026-08-20 16:20:00' },
    { id: 2, title: '璁㈠��到货数量异常', priority: '绱ф一?', assignee: '鍛ㄨ埅', status: '寰呭鐞?', updated_at: '2026-08-21 09:05:00' },
    { id: 3, title: '申请�ュ厖浜у��说明', priority: '浣?', assignee: '鏋楁檽', status: '已�В鍐?', updated_at: '2026-08-21 10:40:00' },
  ],
}
...
function readStore(): StoredData {
  ...
  try {
    ...
    // localStorage 可能被隐私�ā式或浏览�ㄥ畨鍏ㄧ瓥鐣ョ��用，�一回内存存�ㄥ��可�一?
    }
  memoryStore = {
    version: 1,
...
function writeStore() {
  ...
  try {
    ...
    // 写入失败不影响当前�〉闈㈢��续使�ㄥ��存数据�一?
    }
}

function assertTable(tableName: string) {
  if (!TABLE_DEFINITIONS[tableName]) throw new Error(`数据�ㄤ��存在�?{tableName}`)
  return tableName
}
...
function applyWhere(source: Record<string, unknown>[], tableName: string, where?: string) {
  ...
  const predicates = expressions.map(expression => {
    ...
    if (!match || !names.has(match[1])) throw new Error(`过滤�′��不支持：${expression}`)
...
function applyOrder(source: Record<string, unknown>[], tableName: string, orderBy?: string) {
  ...
  if (!match || !fieldNames(tableName).has(match[1])) throw new Error(`排序字段不支持：${orderBy}`)
...
function writableData(tableName: string, data: Record<string, unknown>) {
  ...
  if (!entries.length) throw new Error('娌℃��可写�ョ��数据')
  return entries
}
...
function browserDuplicateProject(projectId: string) {
  ...
  if (!source) throw new Error('应用不存�?')
...
function browserDeleteProject(projectId: string) {
  const projects = readBrowserProjects()
  if (!projects.some(project => project.id === projectId)) throw new Error('应用不存�?')
