...
export const widgetDefinitions: WidgetDefinition[] = [
  ...
  define({ type: 'badge', name: '徽标', description: '角标与数字提示', icon: 'badge', group: 'Element Plus', usage: '用于未读数、状态或数字提醒。', fields: [field('content.text', '展示文字', 'text'), field('content.value', '角标数值', 'text'), field('content.max', '最大显示值', 'number', { min: 1, max: 9999, step: 1 }), field('content.showZero', '显示零值', 'checkbox'), field('style.accent', '强调色', 'color'), field('style.borderRadius', '圆角', 'number', { min: 0, max: 999, step: 1 })] }),
  define({ type: 'tag', name: '标签', description: '分类与状态标记', icon: 'tag', group: 'Element Plus', usage: '支持色调、可关闭状态和关闭事件。', supportedEvents: ['click', 'close'], fields: [field('content.text', '标签文字', 'text'), field('content.tone', '色调', 'select', { options: [option('主要', 'primary'), option('成功', 'success'), option('警告', 'warning'), option('危险', 'danger'), option('信息', 'info')] }), field('content.closable', '可关闭', 'checkbox'), field('style.borderRadius', '圆角', 'number', { min: 0, max: 80, step: 1 })] }),
  define({ type: 'alert', name: '提示框', description: '展示上下文提示', icon: 'alert', group: 'Element Plus', usage: '支持提示类型、详细说明与关闭事件。', supportedEvents: ['close'], fields: [field('content.title', '标题', 'text'), field('content.description', '说明', 'textarea'), field('content.tone', '提示类型', 'select', { options: [option('信息', 'info'), option('成功', 'success'), option('警告', 'warning'), option('危险', 'danger')] }), field('content.closable', '可关闭', 'checkbox'), field('style.borderRadius', '圆角', 'number', { min: 0, max: 80, step: 1 })] }),
  define({ type: 'progress', name: '进度条', description: '展示任务完成进度', icon: 'progress', group: 'Element Plus', usage: '可配置百分比、状态与文字显示。', fields: [field('content.percentage', '百分比', 'number', { min: 0, max: 100, step: 1 }), field('content.status', '状态', 'select', { options: [option('正常', 'normal'), option('成功', 'success'), option('警告', 'warning'), option('异常', 'exception')] }), field('content.showText', '显示文字', 'checkbox'), field('style.accent', '强调色', 'color')] }),
  define({ type: 'switch', name: '开关', description: '布尔状态切换', icon: 'switch', group: 'Element Plus', usage: '支持默认状态、文字和禁用控制。', supportedEvents: ['change'], capabilities: { formField: true, clickable: false }, fields: [field('content.label', '字段标签', 'text'), field('content.activeText', '开启文字', 'text'), field('content.inactiveText', '关闭文字', 'text'), field('content.defaultValue', '默认开启', 'checkbox'), disabledField, field('style.accent', '强调色', 'color')] }),
  define({ type: 'checkbox', name: '多选框', description: '多项选择输入', icon: 'checkbox', group: 'Element Plus', usage: '可配置选项、默认状态和必填校验。', supportedEvents: ['change'], capabilities: { formField: true, clickable: false }, fields: [field('content.label', '字段标签', 'text'), optionField, field('content.defaultValue', '默认选中', 'checkbox'), disabledField, requiredField] }),
  define({ type: 'radio', name: '单选框', description: '单项选择输入', icon: 'radio', group: 'Element Plus', usage: '可配置选项、默认值和必填校验。', supportedEvents: ['change'], capabilities: { formField: true, clickable: false }, fields: [field('content.label', '字段标签', 'text'), optionField, field('content.defaultValue', '默认值', 'text'), disabledField, requiredField] }),
  define({ type: 'datePicker', name: '日期选择', description: '日期或日期时间输入', icon: 'calendar', group: 'Element Plus', usage: '支持日期类型、占位提示和必填校验。', supportedEvents: ['change'], capabilities: { formField: true, clickable: false }, fields: [field('content.label', '字段标签', 'text'), field('content.placeholder', '占位提示', 'text'), field('content.valueType', '选择类型', 'select', { options: [option('日期', 'date'), option('日期时间', 'datetime')] }), disabledField, requiredField] }),
  define({ type: 'pagination', name: '分页', description: '分页导航', icon: 'pagination', group: 'Element Plus', usage: '可配置总数、每页条数和当前页。', supportedEvents: ['change'], fields: [field('content.total', '总条数', 'number', { min: 0, max: 999999, step: 1 }), field('content.pageSize', '每页条数', 'number', { min: 1, max: 100, step: 1 }), field('content.currentPage', '当前页', 'number', { min: 1, max: 99999, step: 1 }), field('style.accent', '强调色', 'color')] }),
  define({ type: 'breadcrumb', name: '面包屑', description: '层级导航', icon: 'breadcrumb', group: 'Element Plus', usage: '使用选项配置导航层级与链接。', supportedEvents: ['click'], fields: [optionField, field('content.separator', '分隔符', 'text'), field('style.accent', '强调色', 'color')] }),
  define({ type: 'tabs', name: '标签页', description: '分组内容导航', icon: 'tabs', group: 'Element Plus', usage: '以纵向列表方式展示标签项并切换内容。', supportedEvents: ['change'], fields: [optionField, field('content.activeKey', '当前选项', 'text'), field('style.accent', '强调色', 'color'), field('style.borderRadius', '圆角', 'number', { min: 0, max: 80, step: 1 })] }),
  define({ type: 'collapse', name: '折叠面板', description: '可展开的信息分组', icon: 'collapse', group: 'Element Plus', usage: '使用选项配置分组并控制默认展开。', supportedEvents: ['change'], fields: [optionField, field('content.expanded', '默认展开', 'checkbox'), field('style.accent', '强调色', 'color')] }),
  define({ type: 'avatar', name: '头像', description: '用户或对象标识', icon: 'avatar', group: 'Element Plus', usage: '支持文字、圆形或方形外观。', fields: [field('content.text', '展示文字', 'text'), field('content.shape', '形状', 'select', { options: [option('圆形', 'circle'), option('方形', 'square')] }), field('style.accent', '强调色', 'color'), field('style.borderRadius', '圆角', 'number', { min: 0, max: 999, step: 1 })] }),
  define({ type: 'icon', name: '图标', description: '图标与辅助文字', icon: 'icon', group: 'Element Plus', usage: '可配置图标名称、辅助文字、颜色和尺寸。', fields: [field('content.iconName', '图标名称', 'text'), field('content.text', '辅助文字', 'text'), field('style.accent', '强调色', 'color'), field('style.fontSize', '字号', 'number', { min: 12, max: 96, step: 1 })] }),
  define({ type: 'link', name: '链接', description: '可跳转的文字链接', icon: 'link', group: 'Element Plus', usage: '支持地址与当前窗口或新窗口打开方式。', fields: [field('content.text', '链接文字', 'text'), field('content.href', '链接地址', 'text'), field('content.target', '打开方式', 'select', { options: [option('当前窗口', '_self'), option('新窗口', '_blank')] }), ...textStyle] }),
  define({ type: 'tooltip', name: '文字提示', description: '悬停说明', icon: 'tooltip', group: 'Element Plus', usage: '配置触发文字、提示内容和展示位置。', fields: [field('content.text', '触发文字', 'text'), field('content.title', '提示内容', 'textarea'), field('content.placement', '位置', 'select', { options: [option('上方', 'top'), option('下方', 'bottom'), option('左侧', 'left'), option('右侧', 'right')] })] }),
  ...
  define({ type: 'modal', name: '弹窗', description: '居中显示的服务组件', icon: 'modal', group: '服务组件', usage: '可由流程打开或关闭，并配置确认和取消按钮。', supportedEvents: ['open', 'close', 'confirm', 'cancel'], capabilities: { container: true }, fields: [field('content.title', '标题', 'text'), field('content.description', '说明', 'textarea'), field('content.confirmText', '确认文字', 'text'), field('content.cancelText', '取消文字', 'text'), field('content.visible', '默认可见', 'checkbox'), field('content.closeOnOverlay', '点击遮罩关闭', 'checkbox'), field('style.accent', '确认按钮色', 'color'), field('style.borderRadius', '圆角', 'number', { min: 0, max: 80, step: 1 })] }),
